#Locate study, make list of SRA numbers 

#Step1 downloading the SRA files to workspace

for x in {1..}   
do
    input=$(awk "NR==$x {print \$0}" ~/rhedata/Table.txt)
    echo "fastq-dump -I $input --split-files --origfmt" >>$x.sh
    echo "gzip -f ${input}_1.fastq ${input}_2.fastq">>$x.sh 
    chmod a+x $x.sh
done

#files are now 1.sh ,2.sh etc.... when run the paired end fastq files will be downloaded and zipped into the workspace.
...
#Step4 merging all the bams into one
...
#Step5 sorting the bam file with picard
...
#Step7 marking indels for local realignment 
ava -Xms2g -Xmx14g -jar /opt/asn/apps/gatk_3.4-46/GenomeAnalysisTK.jar -R rheMac3.masked.fa  -T RealignerTargetCreator -I M_Rhesus.sorted.mkdup.bam  -o M_Rhesus.sorted.mkdup.intervals

#Step8 local realignment of indels 
...
#step9 Extraction of chromosomes of interest. 
...
#step10 Bam2cfg.pl run via bam2cfg.pl -g -h -v 2 -q 40 M_Rhesus.wrg.sorted.mkdup.indel.bam > M_Rhesus.wrg.sorted.mkdup.indel.cfg
#! /usr/bin/perl
#Create a BreakDancer configuration file from a set of bam files

use strict;
use lib "/opt/asn/apps/perl_5.21.1/ActivePerl-5.20.1.2000-x86_64-linux-glibc-2.15-298557/perl/lib/strict.pm";
use warnings;
use lib "/opt/asn/apps/perl_5.21.1/ActivePerl-5.20.1.2000-x86_64-linux-glibc-2.15-298557/perl/lib/warnings.pm";
use Getopt::Std;
use lib "/opt/asn/apps/perl_5.21.1/ActivePerl-5.20.1.2000-x86_64-linux-glibc-2.15-298557/perl/lib/Getopt/Std.pm";
#use Statistics::Descriptive;
use GD::Graph;
use lib "/opt/asn/apps/perl_5.21.1/ActivePerl-5.20.1.2000-x86_64-linux-glibc-2.15-298557/perl/lib/GD/Graph.pm";
use AlnParser;
use histogram;
...
#Step11 breakdancer
/opt/asn/apps/breakdancer/bin/breakdancer-max -d reads M_Rhesus.wrg.sorted.mkdup.indel.cfg > M_Rhesus.wrg.sorted.mkdup.indel.ctx
/opt/asn/apps/breakdancer/bin/breakdancer-max -d reads M_Rhesus2.wrg.sorted.mkdup.indel.cfg > M_Rhesus2.wrg.sorted.mkdup.indel.ctx

#step12 extraction of INV
